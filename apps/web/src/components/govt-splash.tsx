...
export function GovtSplash() {
  ...
  return (
    <div
      ...
      className={`fixed inset-0 z-[100] flex items-center justify-center bg-[#5e1212] transition-opacity duration-300 ${fading ? 'opacity-0' : 'opacity-100'}`}
      style={{
        backgroundImage:
          'radial-gradient(circle at 50% 40%, #7a1a1a 0%, #5e1212 45%, #3a0a0a 100%)',
      }}
    >
      {/* Subtle grid */}
      <div
      ...
      <div className="relative flex flex-col items-center px-6 text-center">
        ...
        <div className="relative">
          <span
            aria-hidden
            className="absolute inset-0 -m-2 rounded-full border-2 border-amber-300/60 motion-safe:animate-ping"
          />
          <span
            aria-hidden
            className="absolute inset-0 -m-6 rounded-full border border-amber-200/30"
          />
          <div className="relative flex h-28 w-28 items-center justify-center rounded-full bg-white shadow-2xl ring-4 ring-amber-300/40">
            <Image
              src="/phi-emblem.png"
        ...
        <p className="mt-7 text-[10px] font-bold uppercase tracking-[0.35em] text-amber-200">
          Ministry of Health · Sri Lanka
        </p>
        <h1 className="mt-2 text-2xl font-extrabold tracking-tight text-white sm:text-3xl">
          PHI&#8209;PRO
        </h1>
        <p className="mt-1 text-xs font-medium text-rose-100/85">
          Digital Health Enforcement &amp; Integrated Intelligence
        </p>

        {/* Loading bar */}
        <div className="mt-7 h-0.5 w-44 overflow-hidden rounded-full bg-white/15">
          <span className="block h-full w-1/3 origin-left animate-[splashbar_1.4s_ease-in-out_infinite] bg-gradient-to-r from-amber-200 via-amber-50 to-amber-200" />
        </div>
        <p className="mt-3 text-[10px] tracking-[0.2em] text-amber-100/70">EST. 1913 &middot; PHI.LK</p>
      </div>
