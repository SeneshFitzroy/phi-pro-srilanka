...
function ReferenceQrGenerator() {
  ...
  const isValid = /^[A-Z]{2,4}-\d{4,12}$/.test(trimmed);
      ...
      <CardContent className="space-y-4 p-6">
        <div>
          ...
          <p className="mt-1 text-xs text-muted-foreground">
            Officers and businesses can paste any PHI-PRO reference here to render a high-contrast QR that this
            page will instantly verify when scanned.
          </p>
        </div>
        ...
        <div className="flex gap-2">
          <Input
            ...
            placeholder="e.g. FP-20250001"
            className="font-mono"
          />
          <Button type="button" disabled={!isValid} onClick={download} className="shrink-0">
        ...
        <div ref={wrapperRef} className="flex flex-col items-center gap-2 rounded-xl border border-dashed border-slate-200 bg-slate-50 p-5 dark:border-slate-700 dark:bg-slate-900/40">
          {isValid ? (
            <>
              <QRCodeCanvas
                value={`https://phipro.lk/public/verify?ref=${encodeURIComponent(trimmed)}`}
                size={180}
                marginSize={2}
                level="H"
                fgColor="#0f172a"
                bgColor="#ffffff"
              />
              <p className="mt-1 font-mono text-xs font-bold text-slate-700 dark:text-slate-300">{trimmed}</p>
              <p className="text-[10px] text-muted-foreground">phipro.lk/public/verify?ref={trimmed}</p>
            </>
          ) : (
            <p className="py-10 text-center text-xs text-muted-foreground">
              Enter a valid reference (prefix + numeric, e.g. <span className="font-mono">FP-20250001</span>) to generate a QR.
            </p>
          )}
        </div>
      </CardContent>
