...
// Seed rows shown while Firestore loads / as fallback
...
function ApprovalsContent() {
  ...
  // Real-time Firestore listener — merges live docs on top of seed rows
  useEffect(() => {
    const q = query(collection(db, 'form_submissions'), orderBy('createdAt', 'desc'));
    const unsub = onSnapshot(q, (snap) => {
      if (snap.empty) return; // keep seed rows if collection is empty
      const live: Submission[] = snap.docs.map((d) => {
        const x = d.data();
        return {
          id:            x.id || d.id,
          officer:       x.officerName || x.submittedBy || '—',
          form:          x.formType    || x.form        || 'Form',
          establishment: x.establishment || x.premisesName || '-',
          area:          x.area        || x.gnDivision  || '—',
          date:          x.createdAt   ? new Date(x.createdAt).toISOString().slice(0, 10) : '—',
          status:        (x.approvalStatus || x.status || 'pending') as Submission['status'],
          grade:         x.grade       || '-',
          isLive:        true,
        };
      });
      setSubmissions(live);
    }, () => { /* Firestore unavailable — keep seed rows */ });
    return () => unsub();
  }, []);
  ...
  const filtered = submissions.filter((s) => {
    ...
    const matchSearch = !searchQ ||
      s.officer.toLowerCase().includes(searchQ.toLowerCase()) ||
      s.form.toLowerCase().includes(searchQ.toLowerCase()) ||
      s.establishment.toLowerCase().includes(searchQ.toLowerCase());
    return matchStatus && matchSearch;
  });

  const pendingCount = submissions.filter((s) => s.status === 'pending').length;

  const setStatus = async (sub: Submission, status: 'approved' | 'rejected') => {
    if (sub.isLive) {
      try {
        await updateDoc(doc(db, 'form_submissions', sub.id), {
      ...
      // seed row: update local state only
  ...
  const approveAll = async () => {
    ...
    await Promise.all(pending.map((s) => setStatus(s, 'approved')));
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <Button variant="outline" size="sm" className="text-green-600" onClick={approveAll}>
          <Check className="mr-1 h-4 w-4" />Approve All Pending
        </Button>
      </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b bg-muted/50">
                {['ID', 'Officer', 'Form', 'Subject', 'Area', 'Date', 'Result', 'Status', 'Actions'].map((h) => (
              ...
              {filtered.map((s) => (
                <tr key={s.id} className="border-b hover:bg-muted/30">
                  ...
                  <td className="px-4 py-3">
                    <span className={`rounded px-2 py-0.5 text-xs font-medium capitalize ${statusColor(s.status)}`}>{s.status}</span>
                  </td>
                  <td className="px-4 py-3">
                    <div className="flex gap-1">
                      ...
                      {s.status === 'pending' && (
                        <>
                          <Button variant="ghost" size="icon" className="h-7 w-7 text-green-600" onClick={() => setStatus(s, 'approved')}><Check className="h-3.5 w-3.5" /></Button>
                          <Button variant="ghost" size="icon" className="h-7 w-7 text-red-600"   onClick={() => setStatus(s, 'rejected')}><X    className="h-3.5 w-3.5" /></Button>
                        </>
                      )}
                    </div>
                  </td>
