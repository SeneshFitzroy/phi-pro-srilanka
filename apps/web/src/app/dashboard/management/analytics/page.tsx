  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-5">
        {[
          { domain: 'Food Safety', total: 280, pct: 36, color: 'border-green-500 bg-green-50 dark:bg-green-950/10' },
          { domain: 'School Health', total: 89, pct: 11, color: 'border-blue-500 bg-blue-50 dark:bg-blue-950/10' },
          { domain: 'Epidemiology', total: 79, pct: 10, color: 'border-red-500 bg-red-50 dark:bg-red-950/10' },
          { domain: 'Occupational', total: 42, pct: 5, color: 'border-yellow-500 bg-yellow-50 dark:bg-yellow-950/10' },
          { domain: 'Administration', total: 137, pct: 18, color: 'border-purple-500 bg-purple-50 dark:bg-purple-950/10' },
        ].map(d => (
          <Card key={d.domain} className={`border-l-4 ${d.color}`}>
            <CardContent className="p-4 text-center">
              <p className="text-2xl font-bold">{d.total}</p>
              <p className="text-xs text-muted-foreground">{d.domain}</p>
              <p className="text-xs font-medium mt-1">{d.pct}% of total</p>
            </CardContent>
          </Card>
        ))}
      </div>
