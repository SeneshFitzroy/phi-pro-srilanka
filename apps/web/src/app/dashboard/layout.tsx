...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <AuthGuard>
          ...
          <div className={cn('flex h-16 items-center border-b border-slate-100 dark:border-slate-800', collapsed ? 'justify-center px-2' : 'justify-between px-4')}>
            {!collapsed && (
              <Link href="/dashboard" className="flex items-center gap-2.5">
                <Image src="/phi-emblem.png" alt="PHI Union of Sri Lanka emblem" width={34} height={34} className="drop-shadow-sm" />
                <div className="flex flex-col leading-none">
                  <span className="text-sm font-extrabold tracking-tight text-slate-900 dark:text-white">PHI-PRO</span>
                  <span className="mt-0.5 text-[10px] font-medium text-slate-400 dark:text-slate-500">PHI Union · Sri Lanka</span>
                </div>
              </Link>
            )}
            {collapsed && (
      ...
      {/* Floating PHI Assistant + voice command navigation — every dashboard page */}
      <AiAssistantFab />
      <VoiceCommands />
    </AuthGuard>
  );
